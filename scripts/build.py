      ...
      fetch('https://zemperica-api.stotrideset7.workers.dev/suggestions')
        ...
        .then(function(d){
          ...
          d.suggestions.slice().reverse().forEach(function(s){
            ...
            if(isAdmin) html+='<div style="text-align:center;margin-top:10px;"><button class="del-suggestion" data-id="'+s.id+'" style="background:none;border:1px solid #2a2a3e;border-radius:6px;color:#555;cursor:pointer;font-size:1.1em;padding:4px 14px;transition:all 0.2s;" title="Obriši">✕</button></div>';
